...
>
  <div class="story-vetrina__media">
    <HeroCarousel images={images} intervalMs={4200} showDots={true} />
  ...
  {#if showHead}
    <div class="story-vetrina__copy">
      ...
      {#if showCta}
        <a class="story-vetrina__cta" href="/vetrina">{$t('home.vetrinaCta')}</a>
      {/if}
    </div>
  {/if}
</section>
...
<style>
  ...
  .story-vetrina__body {
    ...
    font-size: 0.95rem;
    line-height: 1.55;
    color: color-mix(in srgb, var(--color-linen) 75%, transparent);
  }
