...
/** @param {typeof mentors} mentorList */
/** @param {string} slug */
export function getMentorNeighborsFrom(mentorList, slug) {
  const index = mentorList.findIndex((m) => m.slug === slug);
  if (index < 0) return { index: -1, prev: null, next: null };
  return {
    index,
    prev: index > 0 ? mentorList[index - 1] : null,
    next: index < mentorList.length - 1 ? mentorList[index + 1] : null
  };
}

/** @param {string} slug */
export function getMentorNeighbors(slug) {
  return getMentorNeighborsFrom(mentors, slug);
}
